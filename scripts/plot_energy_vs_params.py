...
def plot_molecule(molecule, givens_data, uccsd_data):
    ...
    if givens_data:
        ...
        ax.errorbar(params, errors, yerr=error_bars,
                   fmt='o-', label='Givens (H1-1E)',
                   color='steelblue', markersize=10, linewidth=2.5,
    ...
    if uccsd_data:
        ...
        ax.plot(params, errors, 's-', label='UCCSD (H1-1E)',
               color='darkred', markersize=10, linewidth=2.5, alpha=0.8)
    
    # Format plot
    ax.set_xlabel('Number of Parameters', fontsize=13, fontweight='bold')
    ax.set_ylabel('Energy Error (Ha)', fontsize=13, fontweight='bold')
    ax.set_yscale('log')
    ax.set_title(f'{molecule} - Energy Error vs Number of Parameters',
    ...
    # Save figure
    output_path = os.path.join(OUTPUT_DIR, f'{molecule}_energy_vs_params.png')
